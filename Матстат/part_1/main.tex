...
\solution{
  ...
  \item $\D X_1 = \theta$
  \item $\D X_1 = 1$
  \item $\E X_1 = 1 / \theta \ra \hat{\theta} = \frac{1}{\overline{X}}$. По задаче 10 известно, что оценка асимптотически нормальная с ас-й дисперсией $\theta^2. \psi_{\overline{X}} = \psi_{X_1}^n(t/n) = \left(1 - \frac{it}{n\theta}\rigth)^{-n} \ra $
\end{enumerate}
}
\problem{
