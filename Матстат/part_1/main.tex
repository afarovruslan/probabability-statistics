...
\solution{
  ...
  \begin{enumerate}[label=\alph*]
  \item $p_{X_1}(x) = \theta I_{1}(x) + (1 - \theta)I_{0}(x) \ra u_{\theta}(x) = \frac{I_1(x) - I_0(x)}{p_{X_1}(x)} \ra i(\theta) = \frac{1}{\theta} + \frac{1}{1 - \theta} = \frac{1}{\theta(1 - \theta)}$ \\ \todo{}
  \end{enumerate}
}
...
\solution{
  Рассмотрим $X_1 \sim Bern(\theta)$. Правдоподобие выборки равно $L(\theta) = \theta^{\sum_{i = 1}^n x_n}(1 - \theta)^{n - \sum_{i = 1}^n x_n} \ra$ по критерию факторизации $S(X) = \sum_{i = 1}^n X_n$ является досаточной статистикой.
}
\problem{
...
\solution{
  ...
  \item $L(\theta) = \frac{1}{(2\pi)^{n / 2}} e^{\frac{-\sum_{i = 1}^n (x_i - \theta)^2}{2}} = \frac{1}{(2\pi)^{n / 2}} e^{\frac{-\sum_{i = 1}^n x_i^2}{2}} e^{\theta a - \frac{n\theta^2}{2}} \ra T(X)$ достаточная статистика. $T(X) \sim \ND{\theta}{1/n} $
  \[\E \phi(T) = \int_{\R} \sqrt{\frac{n}{2\pi}} \phi(x) e^{-\frac{n}{2} (x - \theta)^2} dx \]
  \todo{}
