...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item Пусть $T(X) = \max(1, X_{(n)})$, тогда \[q(t) p_t(x) = I_{[1, +\infty)}(t)\frac{1}{t^{n + 2}}I_{0 \le X_{(1)} \le X_{(n)} \le t} = \frac{I_{[T(X), +\infty)}}{t^{n + 2}}\ra \int_{\Theta}q(t) p_t(x)dt = \int_{T(X)}^{+\infty} \frac{dt}{t^{n + 2}} = \] \[ \frac{1}{(n + 1)T(X)^{n + 1}} \ra \hat{\theta} = \frac{n + 1}{n}T(X) \toac \theta \ra \text{ оценка сильно состоятельна}\]
