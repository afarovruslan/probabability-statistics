...
\begin{document}
  ...
  \[ Bin(1, p) = Bern(p), q(t)p_{t}(x) = p^t(1 - p)^{1 - t} \frac{1}{\sqrt{2\pi}}e^{-\frac{\sum_{i = 1}^n (X_i - t)^2}{2}} \ra \] \[
   ...
   \]
   \[
   p_{\theta | X}(t | x) = \frac{p^t(1 - p)^{1 - t}e^{\sum_{i = 1}^n X_i t - \frac{t}{2}}}{pe^{\sum_{i = 1}^n X_i - \frac{1}{2}} + 1 - p} \ra \hat{\theta} = \frac{p e^{\sum_{i = 1}^n X_i - \frac{1}{2}}}{pe^{\sum_{i = 1}^n X_i - \frac{1}{2}} + 1 - p} = 1 - \frac{1 - p}{p(e^{\overline{X} - \frac{1}{2}})^n + 1 - p}
   \]
   По УЗБЧ $\overline{X} \toac \theta \ra $ по теореме о наследовании сходимости $\hat{\theta} \toac \theta \ra$ оценка состоятельна.
  }
  \problem{
  ...
  \begin{enumerate}
    ...
    \]
    Так как $X_{(n)} \toac \theta < 1$ п.н., то $X_{(n)}^n \toac 0 \ra \hat{\theta} \toac \theta \ra$ оценка состоятельна.
...
\[S(X_1, \dots, X_n) = \{ X_{(n)} \le c\theta_0\} \cup \{ X_{(n)} > \theta_0 \}\]
  ...
  Если $\theta > \theta_0$, то $P_{\theta}(X \in S) = F_{X_{(n)}}(c\theta_0) + 1 - F_{X_{(n)}}(\theta_0) = 1 - (1 - \alpha)\left(\frac{\theta_0}{\theta}\right)^n$
  \[
  P_{\theta}(X \notin R) = \frac{1}{\theta^n} \int_{[0, \theta]^n} I_{\overline{R}}(x_1, \dots, x_n) dx_1 \dots dx_n \ge \frac{1}{\theta^n} \int_{[0, \theta_0]^n} I_{\overline{R}}(x_1, \dots, x_n) dx_1 \dots dx_n = \] \[ \left(\frac{\theta_0}{\theta}\right)^n \frac{1}{\theta_0^n} \int_{[0, \theta_0]^n} I_{\overline{R}}(x_1, \dots, x_n) dx_1 \dots dx_n = \left(\frac{\theta_0}{\theta}\right)^n P_{\theta_0}(X \notin R) \ge (1 - \alpha)\left(\frac{\theta_0}{\theta}\right)^n \ra \] \[ P_{\theta}(X \in R) \le 1 - (1 - \alpha)\left(\frac{\theta_0}{\theta}\right)^n = P_{\theta}(X \in S) 
  \]
...
Крамера–Мизеса–Смирнова).
  ...
  \solution{
  \[
  \int_{0}^1 (F_n^*(y) - y)^2dy = \int_{0}^{X_{(1)}} (0 - y)^2 dy + \int_{X_{(1)}}^{X_{(2)}} \left(\frac{1}{n} - y\right)^2 dy + \dots + \int_{X_{(n)}}^1 (1 - y)^2 dy = \] \[\frac{1}{3}\left(X_{(1)}^3 + \sum_{i = 1}^{n - 1}\left(X_{(i + 1)} - \frac{i}{n}\right)^3 - \left(X_{(i)} - \frac{i}{n}\right)^3 + (1 - 1)^3 - (X_{(n)} - 1)^3\right) = \] \[ \frac{1}{3}\left(\sum_{i = 1}^n \left(X_{(i)} - \frac{i - 1}{n}\right)^3 - \sum_{i = 1}^n \left(X_{(i)} - \frac{i}{n}\right)^3\right) = \] \[\frac{1}{3}\left(\sum_{i = 1}^n \frac{1}{n}\left(X_{(i)}^2 - 2X_{(i)}\frac{i - 1}{n} + \frac{i^2 - 2i + 1}{n^2} + X_{(i)}^2 - 2X_{(i)}\frac{i}{n} + \frac{i^2}{n^2} +X_{(i)}^2 - X_{(i)} \frac{2i - 1}{n} + \frac{i^2 - i}{n^2} \right)\right) = 
  \]
  \[\frac{1}{n}\sum_{i = 1}^n \left(X_{(i)}^2 - 2 X_{(i)} \frac{2i - 1}{2n} + \frac{4i^2 -4i + 1}{4n^2} + \frac{4i^2 -8i + 4 + 4i^2 + 4i^2 - 4i - 12i^2 + 12i - 3}{12n^2}\right) = \] \[ \frac{1}{n}\sum_{i = 1}^n \left(X_{(i)} - \frac{2i - 1}{2n}\right)^2 + \frac{1}{n}\sum_{i = 1}^n \frac{1}{12n^2} = \frac{1}{12n^2} + \frac{1}{n}\sum_{k = 1}^n \left(X_{(k)} - (2k-1)/2n\right)^2 \]
  }
  \problem{
...
значимости а) 0.05, б) 0.5, в) 0.8.
  ...
  \begin{enumerate}[label=\alph*]
  \item $\chi^2_{0.95, 9} = 16.9190 \ge \hat{\chi^2} \ra $ мы не отвергаем $H_0$
  \item $\chi^2_{0.5, 9} = 8.3428 \ge \hat{\chi^2} \ra$ мы не отвергаем $H_0$
  \item $\chi^2_{0.2, 9} = 5.3801 \ge \hat{\chi^2} \ra$ мы не отвергаем $H_0$. 
\end{enumerate}
  }
  \problem{
...
где $\theta \in (0, 1/4)$?
  ...
  \solution{
  Найдем ОМП
  \[ L(\theta) = \theta^{840}\theta^{950}(2\theta)^{2200}(1 - 4\theta)^{1010} = c\theta^{3990}(1 - 4\theta)^{1010} \ra l(\theta) = 3990\ln \theta + 1010\ln(1 - 4\theta) + \hat{c}\] \[ \frac{d}{d\theta} l(\theta) = \frac{3990}{\theta} - \frac{4 * 1010}{1 - 4\theta} = 0 \ra \hat{\theta} = \frac{3990}{4 * 5000} = \frac{1995}{2n} \ra \]
  \[
  np_1(\hat{\theta}) = \frac{1995}{2}, np_2(\hat{\theta}) = \frac{1995}{2}, np_3(\hat{\theta}) = 1995, np_4(\hat{\theta}) = 1010 \ra\]
  \[
  \hat{\chi^2} = \frac{2}{1995} \left(\frac{315}{2}\right)^2 + \frac{2}{1995}\left(\frac{95}{2}\right)^2 + \frac{205^2}{1995} + 0 = \frac{6410}{133} \approx 48.2 > \chi^2_{0.98, 3} = 9.83741 \ra
  \]
  ответ - нельзя.
  }
  \problem{
...
Построить доверительный интервал для параметра $\theta$.
  ...
  \]
  \item По аналогии с предыдущей задачей, $\E X = \theta, \D X = \theta$, получаем неравенство $\theta^2 - \theta\left(2\overline{X} + \frac{1}{\alpha n}\right) + \overline{X}^2 < 0 \ra$
  \[\text{Ответ: } \left(\frac{2\overline{X} + \frac{1}{\alpha n} + \sqrt{\frac{4\overline{X}}{\alpha n} + \frac{1}{\alpha^2n^2}}}{2}, \frac{2\overline{X} + \frac{1}{\alpha n} - \sqrt{\frac{4\overline{X}}{\alpha n} + \frac{1}{n^2\alpha^2}}}{2}\right)\]
...
независимы и вычислите распределение статистики $nS^2$.
  ...
  Рассмотрим $\frac{\overline{X} - a}{\sqrt{S^2}} = \sqrt{n}\frac{\overline{X} - a}{\sigma} \frac{1}{\sqrt{\frac{nS^2}{\sigma^2}}} \sim t(n - 1) \ra $ \\
  Ответ: $\left(\overline{X} - t_{\frac{1 + \gamma}{2}}(n - 1)\sqrt{\frac{1}{n}\sum_{i = 1}^n (X_i - \overline{X})^2}, \overline{X} - t_{\frac{1 - \gamma}{2}}(n - 1)\sqrt{\frac{1}{n}\sum_{i = 1}^n (X_i - \overline{X})^2}\right)$
  }
  \problem{
...
б) $H_0 : a_1 = 2a_2$ и $a_1 + 3a_2 = a_3$.
  ...
  \end{pmatrix} + \eps,
  (Z^TZ)^{-1} = 
  diag\left(\frac{2}{n(n+1)}, \frac{6}{m(m+1)(2m + 1)}\right), \]
